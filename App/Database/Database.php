<?php
    ...
    class Database {
        ...
        public function Execute(string $sql, array $params = []) : PDOStatement {
            try {
                ...
                if($e->errorInfo[1] == 1451)
                    Response::Message(FK_ERROR);
                die(Response::Message(GENERAL_ERROR));
            }
        }
         ...
         * @return PDOStatement Objeto PDOStatement
         */
        public function Select(string $join = "", string $where = "", string $order = "", string $fields = "*", array $params = []) : PDOStatement {
        ...
        /**
         * Método responsável por realizar a atualização da quantidade de reagente no banco de dados.
         * @param int $id_reagent ID do reagente
         * @param float $quantity Quantidade do reagente a ser subtraído
         * @return bool Retorna true se a atualização for bem sucedida
         */
        public function UpdateQuantity(int $id_reagent, float $quantity) : bool {
            $sql = "UPDATE reagents SET quantity = quantity - ? WHERE id_reagent = ? LIMIT 1";
            $this->Execute($sql,[
                $quantity,
                $id_reagent
            ]);
